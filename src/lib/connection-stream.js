...
class ConnectionStream extends Readable {
  ...
  constructor (username, type, env = {}) {
    ...
    this.type = type
    this.env = env
    debug(`ConnectionStream for ${this.username} created for `, { type: this.type })
  }
  ...
  _read () {
    ...
    login(this.env).then(() =>
      twitterQuery.getUserConnections(this.username, this.type, this._lastMinPosition)
        .then(connections => {
          ...
          const hasDifferentLastConnection = this._lastReadConnectionId !== lastReadConnectionId
          const hasMoreConnections = !hasZeroConnections && hasDifferentLastConnection
